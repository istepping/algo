...
#############################
def predict(test):
    group, label = read_file(r'data_bill3.txt')
    # 加载系数，归一化处理
    # test = test * np.loadtxt("max_right_weight.txt")
    return knn.classify_knn2(group, label, test, 100)
...
def correct_train(test_radio=0.3, k=20):
    group, label = read_file(r'data_bill3.txt')
    test_num = int(test_radio * group.shape[0])
    train_data = group[test_num:, :]
    train_label = label[test_num:]
    right_count = 0.0  # 正确计数
    for i in range(test_num):
        if knn.classify_knn2(train_data, train_label, group[i, :], k) == label[i]:
            right_count += 1
...
if __name__ == "__main__":
    ...
    # print(predict(np.array([0.56, 0.32, 0.24, 0.18, 0.32, 0.24])))
    k_train2()
    # print(correct_train(0.3,100))
